 ...
 * Read this <a href="https://en.wikipedia.org/wiki/ANSI_escape_code">Wikipedia page</a>) to learn mode about ANSI
 * escape codes.
 * <p>
 * Note that only a subset of the escape codes are currently supported. The most common console colors and styles are
 * currently supported.
 */
public enum AnsiEscCode {
    /**
    ...
    /**
     * The Escape Code to encapsulate.
     */
    ...
    /**
     * Returns a {@code AnsiEscCode} instance based on an escape code.
     *
     * @return a {@code AnsiEscCode} instance based on an escape code.
     * @throws IllegalStateException if no {@code AnsiEscCode} exists for the escape code.
     */
    public static AnsiEscCode fromEscCode(String escCode) {
        var first = Arrays.stream(AnsiEscCode.values())
                .filter(ansiEscCode -> ansiEscCode.escCode.equals(escCode)).findFirst();
        if (first.isPresent()) {
            return first.get();
        }
